...
Rcv_Bytes_LS0:
    ...
    ands   r7, r2
    cmp    r7, r3            @1,   [2]      ;check if input changed
    beq  RCV_sync_bits       @1/2, [3:3/4]
    subs   r5, #1            @1,   [4]      ;dec timeout
    beq  Rcv_Bytes_SyncErro  @1/2, [5]
    ldrh   r7, [r6]          @2,   [0:1]    ;Read input (D- and D+)
    ands   r7, r2
    cmp    r7, r3            @1,   [2]      ;check if input changed
    bne    Rcv_Bytes_LS0     @1,   [3]     
RCV_sync_bits:
...
// Last sync bit
 ...
 ldr    r5, =USB_GPIO_BSRR @2 !DEBUG
 ldr    r4, = 1<<2         @2 !DEBUG set pin
 ldr    r0, = (1<<(2+16))  @2 !DEBUG clear pin
